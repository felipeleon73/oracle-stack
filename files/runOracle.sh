...
echo "Changing oracle user password"
echo oracle:$ORACLE_PWD | sudo chpasswd

if [ -z "$TIME_ZONE" ]; then
  echo "Setting TimeZone"
  sudo rm -rf /etc/localtime;
  sudo ln -s -f /usr/share/zoneinfo/${TIME_ZONE} /etc/localtime;
fi;
echo "Import ENV"
echo '#/bin/bash' > env_path.sh
for i in $(env | egrep "^(PATH=|ORACLE_SID=|ORACLE_PDB=|ORACLE_HOME)")
do
